...
import { api, DOMAIN_COLOR, type Domain } from '../api';
...
export function GraphView() {
  ...
  return (
    <div className="panel">
      ...
      <div style={{ overflow: 'auto', border: '1px solid var(--line)', borderRadius: 8, background: 'var(--bg)' }}>
        <svg viewBox={`0 0 ${W} ${height}`} width="100%" style={{ minWidth: 760, display: 'block' }} role="img" aria-label="Decision graph">
          {DOMAINS.map((d, di) => (
            <text key={d} x={di * COL + COL / 2} y={26} textAnchor="middle" fill={DOMAIN_COLOR[d]} fontSize={14} fontWeight={700}>
              {d}
            </text>
          ))}
          {edges.map((e, i) => {
          ...
          {nodes.map((n) => {
            ...
            return (
              <g key={n.id} transform={`translate(${p.x},${p.y})`} style={{ cursor: 'pointer' }} onClick={() => setSel(isSel ? null : n.id)} opacity={dim ? 0.35 : 1}>
                <circle r={10 + Math.min(8, n.degree * 1.5)} fill={DOMAIN_COLOR[n.domain]} stroke={isSel ? '#fff' : '#0e1020'} strokeWidth={isSel ? 3 : 1.5} />
